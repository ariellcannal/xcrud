...
mb_internal_encoding(Xcrud_config::$mbencoding); // sets multibyte encoding
                                                 // globaly
date_default_timezone_set(@date_default_timezone_get()); // xcrud code not
                                                         // depends on timezone,
                                                         // but this can fix
                                                         // some warnings
...
class Xcrud {
	...
	public $table; // ALTERADO PARA PUBLIC, NECESS�RIO NOS HELPERS CALLBACK'S
	...
	public $defaults = array(); // ALTERADO PARA PUBLIC, NECESS�RIO NOS HELPERS
	                            // CALLBACK'S
	protected $limit = 20;
	...
	protected function subselect_query($name){
		if (isset($this->subselect_query[$name])) {
			...
			$sql = preg_replace_callback('/\{(.+)\}/Uu', array(
					$this,
					'subselect_callback'), $this->subselect[$name]);
		...
		if (! in_array($this->task, array(
			...
			foreach ($this->custom_filter as $filter_name => $opts) {
				...
				if (isset($this->custom_filter_active[$filter_name])) {
					...
					if ($is_totalizer && $filter_name == 'totalizer') {
					} else {
	...
	protected function _build_rel_ins_where($name){
		...
		if ($this->fk_relation[$name]['add_data']) {
			...
			if (is_array($this->fk_relation[$name]['add_data'])) {
				...
				$where_arr[] = preg_replace_callback('/\{(.+)\}/Uu', array(
						$this,
						'rel_where_callback'), $this->fk_relation[$name]['add_data']);
			}
			return implode(' AND ', $where_arr);
		} else {
			return 1;
		...
		if ($this->is_get) {
			...
			if ($this->order_column) {
				...
				$this->order_by = array_merge(array(
						$this->order_column => $this->order_direct), $this->order_by);
			}
			// var_dump($this->order_column);
	...
	protected function create_relation($name, $value = '', $tag = array(), $dependval = false){
		...
		if ($this->relation[$name]['depend_on'] && $dependval === false) {
			...
			if ($this->lists_null_opt) {
				$out .= $this->open_tag(array(
						'tag' => 'option',
						'value' => '')) . $this->lang('null_option') . $this->close_tag('option');
			}
		}
		if ($options) {
	...
	public static function check_url($url, $scr_url = false){
		...
		$doc_root = trim(str_replace('\\', '/', str_replace(array(
				'/public_html',
				'/private_html'), '', $_SERVER['DOCUMENT_ROOT'])), '/');
		$file_dir = trim(str_replace('\\', '/', str_replace(array(
				'/public_html',
				'/private_html'), '', dirname(__file__))), '/');
	 ...
	 *         COMPATIBILIZAÇÃO COM A ARQUITETURA DO CODEIGINITER
	 */
	protected function _get_theme_config(){ // loads theme configuration from
	                                        // ini file
	...
	public static function load_js(){
		...
		if (Xcrud_config::$load_jquery_ui)
			$out .= '<script src="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri . '/jquery-ui/jquery-ui.min.js?' . time() . '"></script>';
		if (Xcrud_config::$load_jcrop) {
			$out .= '<script src="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri . '/jcrop/jquery.Jcrop.min.js?' . time() . '"></script>';
		}
		if (Xcrud_config::$load_bootstrap)
			$out .= '<script src="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri . '/bootstrap/js/bootstrap.min.js?' . time() . '"></script>';
		
		if (Xcrud_config::$editor_url)
			$out .= '<script src="' . Xcrud_config::$editor_url . '?' . time() . '"></script>';
		if (Xcrud_config::$load_googlemap)
			$out .= '<script src="//maps.google.com/maps/api/js?sensor=false&language=' . $language . '&' . time() . '"></script>';
		$out .= '<script src="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri . '/xcrud.js?' . time() . '"></script>';
		...
		if ($this->result_total > $this->limit_list[0]) {
			...
			if ($buttons) {
				...
				$out .= $this->open_tag(array(
						...
						'id' => 'limit_list')) . $this->lang('limit_list_' . $this->limit_list[array_search($limit, $this->limit_list)]);
				...
				foreach ($this->limit_list as $limts) {
					...
					$out .= $this->open_tag(array(
							...
							'data-limit' => $limts)) . $this->lang('limit_list_' . $limts) . $this->close_tag('a');
	...
	protected function render_fields_list($mode, $container = 'table', $row = 'tr', $label = 'td', $field = 'td', $tabs_block = 'div', $tabs_head = 'ul', $tabs_row = 'li', $tabs_link = 'a', $tabs_content = 'div', $tabs_pane = 'div'){
		...
		foreach ($this->fields_output as $key => $item) {
		    ...
		    if(isset($this->validation_required[$item['name']])){
		        $label_class_ = $this->theme_config('details_label_cell').' xcrud-label-required';
		    }
		    else{
		        $label_class_ = $this->theme_config('details_label_cell');
		    }
		    if (isset($this->fields[$item['name']]['tab']) && $this->fields[$item['name']]['tab']) {
		        $tabs_out[$this->fields[$item['name']]['tab']][] = $this->open_tag($row, $row_class) . $this->open_tag($label, $label_class_) . $item['label'] . $this->get_field_tooltip($item['name'], $mode) . $this->close_tag($label) . $this->open_tag($field, $this->theme_config('details_field_cell')) . $item['field'] . $this->close_tag($field) . $this->close_tag($row);
		    } else {
		        ...
		        $raw_out[] = $this->open_tag($row, $row_class_) . $this->open_tag($label, $label_class_) . $item['label'] . $this->get_field_tooltip($item['name'], $mode) . $this->close_tag($label) . $this->open_tag($field, $this->theme_config('details_field_cell')) . $item['field'] . $this->close_tag($field) . $this->close_tag($row);
		    }
		}
		if (isset($this->field_tabs[$mode]) or $this->default_tab !== false) {
			$tabs_header = $this->open_tag($tabs_block, $this->theme_config('tabs_container'), array(
					'class' => 'xcrud-tabs')) . $this->open_tag($tabs_head, $this->theme_config('tabs_header_row'));
			...
			if ($this->default_tab !== false && $raw_out) {
				...
				$tabs_header .= $this->open_tag($tabs_row, $this->theme_config('tabs_header_cell') . ($k == 0 ? ' ' . $this->theme_config('tabs_first_element') : '')) . $this->open_tag($tabs_link, $this->theme_config('tabs_header_link') . ($k == 0 ? ' ' . $this->theme_config('tabs_first_element') : ''), array(
						'href' => '#' . $tabid)) . $this->default_tab . $this->close_tag($tabs_link) . $this->close_tag($tabs_row);
				$tabs_body .= $this->open_tag($tabs_pane, $this->theme_config('tabs_content_pane') . ($k == 0 ? ' ' . $this->theme_config('tabs_first_element') : ''), array(
						'id' => $tabid)) . $this->open_tag($container, $this->theme_config('details_container')) . implode('', $raw_out) . $this->close_tag($container) . $this->close_tag($tabs_pane);
				++ $k;
				$raw_out = array();
			}
				...
				foreach ($this->field_tabs[$mode] as $key => $tabname) {
					if (isset($tabs_out[$tabname])) {
						...
						$tabs_header .= $this->open_tag($tabs_row, $this->theme_config('tabs_header_cell') . ' ' . $class) . $this->open_tag($tabs_link, $this->theme_config('tabs_header_link') . ' ' . $class, array(
								'href' => '#' . $tabid)) . $tabname . $this->close_tag($tabs_link) . $this->close_tag($tabs_row);
						$tabs_body .= $this->open_tag($tabs_pane, $this->theme_config('tabs_content_pane') . ' ' . $class, array(
								'id' => $tabid)) . $this->open_tag($container, $this->theme_config('details_container')) . implode('', $tabs_out[$tabname]) . $this->close_tag($container) . $this->close_tag($tabs_pane);
						++ $k;
					}
				}
			}
			...
			if ($this->nested_rendered && Xcrud_config::$nested_in_tab) {
				foreach ($this->nested_rendered as $tabname => $content) {
					...
					$tabs_header .= $this->open_tag($tabs_row, $this->theme_config('tabs_header_cell') . ' ' . $class) . $this->open_tag($tabs_link, $this->theme_config('tabs_header_link') . ' ' . $class, array(
							'href' => '#' . $tabid)) . $tabname . $this->close_tag($tabs_link) . $this->close_tag($tabs_row);
					$tabs_body .= $this->open_tag($tabs_pane, $this->theme_config('tabs_content_pane') . ' ' . $class, array(
							'id' => $tabid)) . $content . $this->close_tag($tabs_pane);
					++ $k;
			...
			for($i = 1; $i <= $this->search_lines; $i ++) {
				...
				if (Xcrud_config::$search_all) {
					$optlist[] = $this->open_tag('option', '', array(
							'value' => '')) . $this->lang('all_fields') . $this->close_tag('option');
				...
				if ($this->search_default === false && ! $this->search_columns) {
					$out .= $this->open_tag(array(
							'tag' => 'input',
							'type' => 'hidden'), 'xcrud-columns-select ' . $this->theme_config('search_fieldlist'), $attr);
				} else {
			...
			case 'timestamp' :
				if (! isset($fieldlist['date']) or $field == @$this->search_submit[$line]['column']) {
					...
					$fieldlist['date'] .= $this->open_tag('option', '', array(
							'value' => '')) . $this->lang('choose_range') . $this->close_tag('option');
	...
	protected function render_grid_head($row = array('tag' => 'tr'), $item = array('tag' => 'th'), $arrows = array('asc' =>
		...
		if ($this->is_mass_remove || count($this->mass_edit)) {
			$out .= $this->open_tag($item, 'xcrud-mass') . $this->open_tag('span', $this->theme_config('mass_checkbox_header_container')) . $this->single_tag('input', 'xcrud-mass ' . $this->theme_config('mass_checkbox_header'), array(
					'type' => 'checkbox')) . $this->single_tag('label') . $this->close_tag('span') . $this->close_tag($item);
		}
		if ($this->is_numbers) {
		...
		if ($this->result_list) {
			foreach ($this->result_list as $key => $row) {
				...
				if ($this->is_mass_remove || count($this->mass_edit)) {
					...
					if (! $this->table_ro && ($this->is_edit($row) || $this->is_remove($row))) {
						$out .= $this->open_tag('span', $this->theme_config('mass_checkbox_container')) . $this->single_tag('input', 'xcrud-mass xcrud-data ' . $this->theme_config('mass_checkbox'), array(
								...
								'value' => $row['primary_key'])) . $this->single_tag('label') . $this->close_tag('span');
					}
					$out .= $this->close_tag($item);
				}
				if ($this->is_numbers) {
				...
				if (($this->is_edit || $this->is_remove || $this->is_view || $this->buttons || $this->is_duplicate || $this->grid_restrictions) && $this->task != 'print' && $this->buttons_position == 'left') {
					$out .= $this->open_tag($item, 'xcrud-actions' . ((Xcrud_config::$fixed_action_buttons) ? '' : ''), $this->_cell_attrib(false, false, false, $row, false, $row_color, $row_class));
				...
				if (($this->is_edit || $this->is_remove || $this->is_view || $this->buttons || $this->is_duplicate || $this->grid_restrictions) && $this->task != 'print' && $this->buttons_position == 'right') {
					$out .= $this->open_tag($item, 'xcrud-actions' . ((Xcrud_config::$fixed_action_buttons) ? ' xcrud-actions-fixed' : '') . (Xcrud_config::$fixed_action_buttons ? ' xcrud-fix' : ''), $this->_cell_attrib(false, false, false, $row, false, $row_color, $row_class));
					$out .= $this->_render_list_buttons($row);
					$out .= $this->close_tag($item);
				}
			...
			$out .= $this->open_tag($row_tag, 'xcrud-row') . $this->open_tag($item, '', array(
					'colspan' => $j)) . $this->lang('table_empty') . $this->close_tag($item) . $this->close_tag($row_tag);
		}
		return $out;
	}
	...
	protected function render_button($name = '', $task = '', $after = '', $class = '', $icon = '', $mode = '', $primary = ''){
		...
		if (! isset($this->hide_button[$name])) {
			...
			if ($icon && ! $this->is_rtl) {
				$out .= $this->open_tag(array(
						'tag' => 'i',
						'class' => $icon)) . $this->close_tag('i') . ' ';
			}
			$out .= $this->open_tag(array(
					'tag' => 'span')) . $this->lang($name) . $this->close_tag('span');
			if ($icon && $this->is_rtl) {
				$out .= ' ' . $this->open_tag(array(
						'tag' => 'i',
						'class' => $icon)) . $this->close_tag('i');
			...
			if ($this->custom_lists) {
				...
				foreach ($modals as $tag_id => $cfg) {
					...
					$out .= $this->open_tag('li', 'active') . $this->open_tag('a', '', array(
							'data-toggle' => 'tab',
							'href' => '#filtros_' . $tag_id)) . 'Filtros' . $this->close_tag('a') . $this->close_tag('li');
					if ($this->table == "propostas")
						$out .= $this->open_tag('li', '') . $this->open_tag('a', '', array(
								'data-toggle' => 'tab',
								'href' => '#filtrosAdicionais_' . $tag_id)) . 'Filtros Adicionais' . $this->close_tag('a') . $this->close_tag('li');
					$out .= $this->open_tag('li', '') . $this->open_tag('a', '', array(
							'data-toggle' => 'tab',
							'href' => '#colunas_' . $tag_id)) . 'Colunas' . $this->close_tag('a') . $this->close_tag('li');
					$out .= $this->close_tag('ul');
					...
					if ($this->table == "propostas") {
						...
						$out .= $this->open_tag('option', '', array(
								'value' => 0)) . 'Selecione' . $this->close_tag('option');
					...
					$out .= $this->open_tag('button', 'btn btn-default', array(
							'type' => 'button',
							'data-dismiss' => 'modal',
							'aria-hidden' => 'true')) . $this->lang('Cancelar') . $this->close_tag('button');
					if ($cfg['id'] != "")
						$out .= $this->open_tag('button', 'btn btn-danger remove', array(
								'type' => 'button')) . $this->lang('remove') . $this->close_tag('button');
					$out .= $this->open_tag('button', 'btn btn-primary save', array(
							'type' => 'button')) . $this->lang('save') . $this->close_tag('button');
					$out .= $this->close_tag('div');
					$out .= $this->close_tag('div');
					$out .= $this->close_tag('div');
					$out .= $this->close_tag('div');
	...
	public function render_mass_edit_actions(){
		if ($this->is_mass_remove || count($this->mass_edit)) {
			...
			$out .= $this->open_tag('select', 'xcrud-mass-select not_select2 not_select2 xcrud-data ' . $this->theme_config('mass_select'), array(
					'name' => 'mass_task'));
			$out .= $this->open_tag('option', '', array(
					'value' => 0)) . $this->lang('mass_action') . $this->close_tag('option');
			if (count($this->mass_edit))
				$out .= $this->open_tag('option', '', array(
						'value' => 1)) . $this->lang('mass_edit') . $this->close_tag('option');
			if ($this->is_mass_remove)
				$out .= $this->open_tag('option', '', array(
						'value' => 2)) . $this->lang('mass_remove') . $this->close_tag('option');
			$out .= $this->close_tag('select');
			$out .= $this->open_tag('a', 'xcrud-action ' . $this->theme_config('mass_apply_button'), array(
					'data-task' => 'mass',
					'href' => 'javascript:;',
					'data-after' => 'list',
					'data-confirm' => $this->lang('mass_apply_confirm'))) . $this->lang('mass_apply') . $this->close_tag('a');
			$out .= $this->close_tag('div');
			return $out;
		}
	}
		...
		if ($this->is_mass_remove || count($this->mass_edit)) {
			...
			foreach ($this->mass_edit as $field => $fitem) {
				...
				$out .= $this->open_tag('label', 'control-label', array(
						'for' => $field)) . $lbl . $this->close_tag('label');
				$func = 'create_' . $type;
				unset($attr);
